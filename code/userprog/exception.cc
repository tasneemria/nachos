...
void ExitProcess()
{
	...
	if(processTable->numProcesses == 0)
		interrupt->Halt();
	else
...
void pageFaultExceptionHandler(){
	unsigned int faultingAddr = machine->ReadRegister(BadVAddrReg);
	unsigned int vpn = virtualAddress / PageSize;

	stats->numPageFaults++;
	currentThread->space->loadIntoFreePage(faultingAddr, vpn);
}
